...
# Add multiple modules. '--all' tells command that every
# remaining argument is a module name.
$ sudo modprobe --all rdma_cm grubby
```
...
```bash
# replace 'my-app.conf' with a descriptive file name
sudo touch /etc/modules-load.d/my-app.conf
echo my_module | sudo tee /etc/modules-load.d/my-app.conf
```
